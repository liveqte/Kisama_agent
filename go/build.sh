...
rm -f "$OUTPUT_DIR/agent-windows-amd64.exe" "$OUTPUT_DIR/agent-windows-arm64.exe"
...
if command -v zig >/dev/null 2>&1 && command -v patchelf >/dev/null 2>&1; then
    echo "🌟 跨平台构建工具链 (Zig + patchelf) 准备就绪，开始构建 7 个全平台产物..."
    echo ""
    ...
    echo "   ➜ 降级模式：构建当前宿主机平台 + Windows 平台的纯 Go 二进制..."
    echo ""
